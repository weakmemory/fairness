...
Definition uniq_ts (s : RA_label -> Prop) :=
...
Lemma tr_ra_nodup t (ND : trace_nodup (trproj t))
...
Proof using.
  ...
  destruct My; desf.
  assert (xl = yl) by eauto using tr_ra_nodup.
  desf; lia.
Qed.
...
Proof using.
  ...
  - assert (xl = yl0) by eauto using tr_ra_nodup.
...
Proof using.
  ...
  { assert (yl0 = yl) by eauto using tr_ra_nodup; subst.
...
Lemma RA_mem t mem (INIT : mem 0 = Sinit) d
      (STEP : forall i (LTi: NOmega.lt_nat_l i (trace_length t)),
          RA_step (mem i) (trace_nth i t d) (mem (S i)))
      n (LT: NOmega.lt_nat_l n (trace_length t)) :
  fst (mem n) ≡₁ Minit ∪₁
      ⋃₁ i < n, RA_wmsg ↑₁ (RA_is_w ∩₁ eq (trace_nth i t d)).
Proof using.
  induction n.
...
Lemma RA_uniq_ts t (T: LTS_trace ra_lts t) :
...
Proof using.
    ...
    try rewrite EQl in *; desf.
  {
    ...
    all: do 2 eexists; eapply RA_mem; unfolder; ins; eauto.
...
Proof using.
  ...
  destruct (IndefiniteDescription.constructive_indefinite_description);
    clear e; ins; desc; f_equal; eapply tr_ra_nodup; eauto.
Qed.

Lemma RA_view t mem (INIT : mem 0 = Sinit) d
      (STEP : forall i (LTi: NOmega.lt_nat_l i (trace_length t)),
          RA_step (mem i) (trace_nth i t d) (mem (S i)))
...
Lemma view_le_same_tid t (TR: LTS_trace ra_lts t)
...
Proof using.
  ...
  eapply view_le_trans, RA_step_view_ext; eauto.
  eapply view_le_trans, RA_view with (d := d); eauto.
...
Proof using.
  ...
  assert (xl = trace_nth x0 t (deflabel))
    by (eapply tr_ra_nodup; eauto with hahn).
  ...
  forward eapply RA_mem with (d := deflabel) as X; eauto.
...
Proof using.
  ...
  destruct H;
    [ eapply RA_sb_trord | eapply RA_rf_trord ];
    eauto using RA_uniq_ts.
Qed.

Lemma RA_le_view_sb t
      (TR: LTS_trace ra_lts t)
      (WF: trace_wf (trproj t)) x y
      (REL: (⦗is_init ∪₁ trace_elems (trproj t)⦘ ⨾ ext_sb ⨾
             ⦗is_init ∪₁ trace_elems (trproj t)⦘) x y) :
...
Proof using.
       ...
       rewrite EQ0 in *; ins; desf; done.
  {
                 ...
                 trace_nth n t (deflabel)).
    {
      destruct (trace_nth y t (deflabel)) eqn: M; ins; desf.
      all: rewrite <- M.
      all: eapply RA_uniq_ts with (x := _); eauto using match_ev with hahn.
      all: rewrite M, EQl; ins.
    }
    eapply trace_nodup_filter_inj; eauto using trace_nodup_mapE with hahn.
    rewrite EQy, EQl; ins.
...
Proof using.
  ...
  destruct H;
    [ eapply RA_le_view_sb | eapply RA_le_view_rf ]; eauto.
Qed.
...
Proof using.
  ...
  rewrite Nat.lt_succ_r, Nat.le_lteq in MEM; desf.
    apply IHn; ins; eapply RA_mem; eauto;
...
Lemma view_of_write t (TR : LTS_trace ra_lts t) y yl
      (My : match_ev y yl)
      (ELy : trace_elems t yl)
      (Wy : is_w y) :
...
Proof using.
  ...
  assert (RF:= REL); apply RA_view_rf_loc in REL; ins.
  assert (LEQ: loc x = loc z) by (unfold myrf in *; desf).
  rewrite LEQ.
  assert (CO:= REL1); apply RA_view_co in REL1; ins; desf; try lia.
  ...
  eapply RA_uniq_ts in VEQ; eauto.
...
Proof using.
  ...
  }
  {
    ...
    }
    {
      eapply trace_in_nth with (d := deflabel) in ELy; desf.
      unfold valw, valr.
      forward eapply RA_mem with (n := n) (d := deflabel) as X;
        ins; eauto.
      unfold Minit in X.
      ...
      all: forward eapply U with
          (e := RA_event (ThreadEvent t0 i lab) tstamp view)
...
Proof using.
  ...
  forward eapply RA_mem with (d := deflabel) as Y; ins; eauto.
  unfold Minit in Y.
...
Lemma RAG_consistent t
      (TR: LTS_trace ra_lts t)
      (WF : trace_wf (trproj t)) :
  ra_consistent {| acts := is_init ∪₁ trace_elems (trproj t);
                   rf := myrf t;
                   co := myco t |}.
Proof using.
  destruct (LTS_traceE _ _ TR) as (mem & INIT & STEP); desf.
  red; unfolder; splits; ins; desf.
  - apply RA_hb_trord in H; ins; lia.
  - apply RA_le_view_hb in H; ins.
    apply RA_view_co in H0; auto.
    specialize (H (loc z)); lia.
  - apply RA_le_view_hb in H; ins.
    apply RA_view_fr in H0; auto.
    specialize (H (loc z)); lia.
  - apply RA_view_co in H1; auto.
    destruct (classic (z0 = x)); desf; try lia.
    forward eapply RA_view_fr as Y; eauto.
    red; unfolder; ins; splits; try intro; desf; eauto.
    assert (EQl: loc z0 = loc z) by (red in H0; desf).
    assert (EQl': loc x = loc z) by (red in H; desf).
    rewrite EQl, EQl' in *; lia.
Qed.
...
Lemma fsupp_co t (TR : LTS_trace ra_lts t)
      (ND : trace_nodup (trproj t)) :
  fsupp (myco t).
Proof using.
  red; ins.
                          ...
                          ∩₁ (fun x => write_ts x < S tstamp))).
  {
    ...
    eapply RA_uniq_ts; try eapply match_evE; eauto; congruence.
  }
  ...
  assert (yl = RA_event (ThreadEvent t0 i lab) tstamp view)
    by (eapply tr_ra_nodup; vauto); desf.
  specialize (FIN xl); destruct Mx; desf; ins.
  right; rewrite in_map_iff; eexists (RA_event _ _ _);
    split; ins; apply FIN; splits; ins.
Qed.

Lemma fsupp_rfr s t (TR : LTS_trace_param ra_lts s t)
      (FAIR : run_fair s t)
      (ND : trace_nodup (trproj t)) :
                          ...
                          ∩₁ (fun x => view_of x (loc_l lab) < S tstamp))).
  {
      ...
      apply FAIR0; eauto. }
    {
      ...
      eapply RA_view_mono
        ...
        eauto.
      forward eapply RA_step_view_ext with (lab := fl n0) as [_ VEQ]; eauto.
      ...
      forward eapply RA_step_view_ext with (lab := fl n1) as [Y YY]; eauto.
    ...
    eapply RA_view_mono
      ...
      eauto.
    edestruct RA_step_view_ext with (lab := fl n1) as [X _]; eauto.
    specialize (X (loc_l lab)).
    forward eapply (RA_mem (trace_inf fl))
      with (n := j) (d := deflabel) as MEM; ins; eauto.
    apply proj2 in MEM.
    rewrite set_subset_union_l in MEM; apply proj2 in MEM.
    
    specialize (MEM (RA_wmsg (fl n))); specialize_full MEM.
    ...
    all: eapply F0 with (_, _), RAstep_internal; try eassumption; ins; lia. 
  }
  assert (LTS_trace ra_lts t) as LTSTR.
  { red. desf; exists s; apply TR. }
  clear FAIR.
  unfolder in FIN; desc.
  exists (map proj_ev findom); ins; desf; in_simp.
  assert (LEQ: loc z = loc_l lab) by (red in REL1; desf; ins).
  eapply RA_view_co in REL1; ins.
  assert (RF := REL0); eapply RA_view_rf_loc in REL0; ins; desf.
...
Lemma RA_op_implies_decl s t
      (TR: LTS_trace_param ra_lts s t)
      (WF : trace_wf (trproj t))
      (TWF : trace_no_init_tid (trproj t))
      (FAIR: run_fair s t) :
  exists G,
...
Proof using.
  ...
  assert (LTS_trace ra_lts t) as LTSTR.
  { red. desf; exists s; apply TR. }
  assert (U : uniq_ts (trace_elems t)) by eauto using RA_uniq_ts.

  exists {| acts := is_init ∪₁ trace_elems (trproj t) ;
            rf := myrf t ; co := myco t |}; splits; ins;
    eauto using RAG_wf, RAG_rfcomplete, RAG_consistent.
  { unfolder; intuition.
    rewrite trace_elems_trproj in *; desf.
    destruct H1; desf. }
  split; ins; eauto using fsupp_co.
  rewrite frE; ins; eauto using RAG_wf, RAG_rfcomplete,
                    ra_rmw_atomicity, RAG_consistent.
