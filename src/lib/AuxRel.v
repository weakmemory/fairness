...
Lemma set_minus_subset {A: Type} (S1 S2: A -> Prop):    
  S1 \₁ S2 ⊆₁ S1.
Proof. basic_solver. Qed. 

Lemma set_inter_subset {A: Type} (S1 S2: A -> Prop):    
  S1 ∩₁ S2 ⊆₁ S1.
Proof. basic_solver. Qed. 
...
Lemma pow_rel_explicit {A: Type} (r: relation A) (x y: A) (n: nat):
  r ^^ n x y
  <->
  exists l, nth_error l 0 = Some x /\ nth_error l n = Some y /\
       forall i (DOM: i < n), exists a b,
           nth_error l i = Some a /\
           nth_error l (i + 1) = Some b /\
           r a b.
Proof.
  generalize dependent x. induction n.
  { ins. split; ins.
    { red in H. desc. subst. exists (cons y nil). splits; vauto. ins. lia. }
    { desc. destruct l; [by vauto| ]. inversion H. inversion H0. by subst. }
  }
  intros. split.
  { intros REL. eapply same_relation_exp in REL.
    2: { rewrite pow_S_begin. auto. }
    destruct REL as (z & Rxz & Rzy).
    specialize (IHn z). destruct IHn as [IHn _]. specialize_full IHn; auto. desc.
    exists (x :: l). splits; try by vauto.
    ins. destruct i; [by vauto| ].
    apply Lt.lt_S_n in DOM. simpl. by apply IHn1. }
  { intros (l & L). destruct l.
    { by desc. }
    simpl in L.  desc. inv L. clear L.
    pose proof (L1 0) as B. specialize_full B; [lia| ].
    desc. simpl in B. inv B. clear B.
    specialize (IHn b). destruct IHn as [_ IHn]. specialize_full IHn.
    { exists l. splits; try by vauto. ins.
      specialize (L1 (S i)). specialize_full L1; [lia| ].
      desc. exists a0, b0. splits; vauto. }
    apply pow_S_begin. eexists. splits; eauto. }    
Qed.
